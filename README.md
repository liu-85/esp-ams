...
**OTA 上传在预览里是"只演练、不落盘"的**：把 `.ams` 拖进去会真的走一遍
分包接收 + CRC32 校验，然后告诉你"通过 N 个文件"，但文件写在一个临时目录里，
校验完就删掉 —— 不会覆盖你的 `python_code/`。想试"整机 BIN 会被拒绝"的效果，
直接把 `dist/esp32c3-ams-firmware.bin` 传上去即可，应当看到提示改走 USB。

> 预览服务是多线程的。因为它要一边收几百 KB 的更新包，一边响应页面每 2 秒
> 的 `/status` 和 `/log` 轮询；单线程的话轮询会被上传挡住，浏览器等不到响应
> 就会把上传连接一起掐掉，看起来像"升级失败"。
